...
class gameGUI:
    ...
    def startCanvas(self) -> None:
        ...
        self.tkobject.configure(bg='#bcd6f5')
    ...
    def setMenu(self) -> None:
        ...
        getinfobutton = tk.Button(self.tkobject, text='Enter', command=lambda: self.gethostinfo(iptext, portentry, username, getinfobutton))
        getinfobutton.place(x=230, y=670)
    

    def ipEntry(self) -> None:
        '''Creates the entry to enter in your ip address.'''
        iplabel = tk.Label(self.tkobject, text="Enter your ip address: ", font=('Arial', 13), bg='#cd97f7')
    ...
    def portEntry(self) -> None:
        ...
        port = tk.Label(self.tkobject, text="Enter your port: ", font=('Arial', 13), bg='#cd97f7')
    ...
    def usernameEntry(self) -> None:
        ...
        usernamelabel = tk.Label(self.tkobject, text="Enter your username: ", font=('Arial', 13), bg='#cd97f7')
    ...
    def StatsMenu(self) -> None:
        ...
        statsframe = tk.Frame(self.tkobject, height=200, width=350, bg='#cd97f7')
    ...
    def gethostinfo(self, ipobject, portobject, usernameobject, infoobject) -> None:
        ...
        try:
            ...
            try:
                self.socketobject.connect(infotuple)
    ...
    def play_again(self, draworwin):
        '''Asks player1 if they want to play again, ends both programs if not.
        
        Args:
            Takes in a string that indicates whether it was a draw or a win that ended the game.
        '''
        recentwinner = self.boardobject.getRecent_player()
        if draworwin == 'win':    
            if recentwinner == self.user_name:
                result = messagebox.askyesno(title='Play Again?', message='You won! Do you want to play again?')
            else:
                result = messagebox.askyesno(title='Play Again?', message='You lost. Do you want to play again?')
        else:
            result = messagebox.askyesno(title='Play Again?', message='It was a draw. Do you want to play again?')
        if not result:
            self.StatsMenu()
            self.socketobject.send(b'Fun Times')
            self.disablebuttons()
        else:
            self.tkobject.after(200, self.socketobject.send(b'Play Again'))
            self.StatsMenu()
            self.enableButtons()
